...
export const normalizeGrowthScenario = (
  scenario: GrowthScenario,
): GrowthScenario => ({
  ...scenario,
  ...
  compoundingPerYear: [1, 2, 4, 12].includes(scenario.compoundingPerYear)
    ? scenario.compoundingPerYear
    : 1,
...
export const projectGrowthScenarioAtYear = (
  input: GrowthScenario,
  year: number,
): GrowthProjection => {
  ...
  const netAnnualRate =
    (scenario.annualRate / 100) * taxMultiplier - scenario.annualFeeRate / 100;
  ...
  if (scenario.reinvestEarnings) {
    ...
    endingValue =
      scenario.principal *
      Math.pow(
        Math.max(0, 1 + periodicRate),
        scenario.compoundingPerYear * horizon,
      );
  } else {
    endingValue =
      scenario.principal + scenario.principal * netAnnualRate * horizon;
  }

  const safeEndingValue = Math.max(
    0,
    Number.isFinite(endingValue) ? endingValue : scenario.principal,
  );
  return {
    year: horizon,
    endingValue: safeEndingValue,
    earnings: safeEndingValue - scenario.principal,
  };
};

export const getGrowthScenarioResult = (
  input: GrowthScenario,
): GrowthScenarioResult => {
  const scenario = normalizeGrowthScenario(input);
  const projectionYears = Array.from(new Set([1, 3, 5, scenario.years])).sort(
    (left, right) => left - right,
  );
  const projections = projectionYears.map((year) =>
    projectGrowthScenarioAtYear(scenario, year),
  );
  const selectedProjection =
    projections.find((projection) => projection.year === scenario.years) ??
    projections[0];
  const effectiveAnnualRate =
    scenario.principal > 0 && selectedProjection.year > 0
      ? Math.pow(
          selectedProjection.endingValue / scenario.principal,
          1 / selectedProjection.year,
        ) - 1
      : 0;
  const liquidityLabel =
    scenario.liquidity === "anytime"
      ? "High"
      : scenario.liquidity === "limited"
        ? "Medium"
        : "Low";
  const accessLabel =
    scenario.liquidity === "anytime"
...
export const productDefaults: Record<
  GrowthProductType,
  Pick<
    GrowthScenario,
    | "annualRate"
...
/** Each line stops at its selected term; comparisons share one time/value scale. */
export function growthComparison(scenarios: GrowthScenario[]) {
  const normalized = scenarios.map(normalizeGrowthScenario);
  const horizon = Math.max(1, ...normalized.map((s) => s.years));
  const series = normalized.map((scenario) => ({
    scenario,
    points: Array.from({ length: scenario.years + 1 }, (_, year) =>
      projectGrowthScenarioAtYear(scenario, year),
    ),
  }));
  const maximum = Math.max(
    1,
    ...series.flatMap((s) => s.points.map((p) => p.endingValue)),
  );
  return { horizon, maximum, series };
}
export const GROWTH_COLORS = ["#009caf", "#35b878", "#8d6bd1", "#dc8b26"];
