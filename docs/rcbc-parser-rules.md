...
- Treat `DiDi` / `DiDi Tianjin` as a ride-hailing merchant: `Transport`, expense. Never infer a transfer from its capitalized location-style descriptor.
- Treat `Naixue` and `Pancake House` as `Food & Dining` when the merchant string is otherwise clear.
