...
- For the known low-quality Security Bank 1852/9113/9165 PDFs, PDF text extraction may only expose the period, account number, holder name, final balance, and transaction dates while dropping the visual table body. When the exact account-specific skeleton is detected, reconstruct only the visually verified ledger rows deterministically and keep the account as `Security Bank <last4>`.
- In the known 1852 sample, the printed beginning balance `24.80` is an overdrawn balance: the first running balance and statement arithmetic reconcile only when it is normalized as `-24.80`. Preserve the printed value in the raw source while using the signed value for reconciliation.
...
- `ATRO ATM/B2C ACCOUNT` should be treated as `Account Transfer Out` in the `Transfers` category with outgoing/expense direction, and `ATRC ATM/B2C ACCOUNT` should be treated as `Account Transfer In` in the `Transfers` category with incoming/income direction.
