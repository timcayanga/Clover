...
- Found a device-only Android keyboard issue: Login actions could not be scrolled fully above the keyboard. Added Android-only keyboard avoidance around authentication; iOS keeps its existing automatic scroll insets. Release APK rebuilt and installed; Login and Sign Up actions now scroll fully above the open Android keyboard. Before/after screenshots are saved in the evidence directory.
- The full release gate initially stopped at Expo dependency compatibility because new SDK 57 patch versions were required. Applied the recommended patch updates without changing SDK or React Native versions. TypeScript and dependency compatibility now pass; the complete release gate passed after merging the latest staging billing/analytics changes and regenerating local Prisma types.
...
- Android rebuild required restoring Java 17 in the generated local Gradle daemon configuration after Android Studio selected Java 25. Final arm64 Release build succeeded; no project authentication, account data or production settings were changed.

Validation for this follow-up: `npm run qa:prepush` passed (including web TypeScript, regression suites, native TypeScript/dependency checks, iOS/Android bundles and production web build). Android Release APK built and the authentication keyboard fix was verified on API 35. The installed iOS simulator app predates the final Expo patch refresh; final signed store builds and authenticated parity remain outstanding.
