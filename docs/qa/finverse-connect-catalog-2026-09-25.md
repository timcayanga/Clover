...
- Country tiles now come only from eligible returned bank connections in Clover's supported Connect markets; empty countries and unknown country codes are omitted.
...
- Country labels use full names. Supplied country PNGs are shipped on web and bundled on native. Thailand retains its flag emoji if an eligible connection is returned because no Thailand image was supplied.
