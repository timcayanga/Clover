...
import { occupiesCampaignPlace } from "../../../../../../shared/switch-campaign";
...
export async function POST(request:Request){try{assertTrustedRequestOrigin(request);const admin=await requireAdminAuth("entitlements");const body=z.discriminatedUnion("action",[
 z.object({action:z.literal("configure"),status:z.enum(["draft","active","paused","ended"]),capacity:z.number().int().min(1).max(100),endsAt:z.string().datetime().nullable(),reason:z.string().trim().min(3).max(500)}),
 ...
 if(body.action==="configure")await growthTransaction(async tx=>{const before=await tx.switchCampaign.findUnique({where:{id:campaignId()}});if(before?.status==="ended"&&body.status!=="ended")throw new Error("An ended campaign cannot restart.");const places=await tx.switchApplication.findMany({where:{campaignId:campaignId()},select:{activatedAt:true,status:true,claimBy:true}});if(body.capacity<(before?.redeemedCount??0)+places.filter(a=>!a.activatedAt&&occupiesCampaignPlace(a,new Date())).length)throw new Error("Capacity cannot be below reserved and redeemed places.");if(body.status==="active"&&body.endsAt&&new Date(body.endsAt)<=new Date())throw new Error("End date must be in the future.");const data={status:body.status,capacity:body.capacity,endsAt:body.endsAt?new Date(body.endsAt):null};await tx.switchCampaign.upsert({where:{id:campaignId()},create:{id:campaignId(),environment:getCurrentUserEnvironment(),...data},update:data});await tx.growthAudit.create({data:{actorId:admin.userId,targetId:campaignId(),action:"switch_configure",reason:body.reason,after:data}});});
