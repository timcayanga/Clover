...
import { matchingBankAccounts, bankTransactionMatches } from "@/lib/finverse-matching";
...
const importAccount = async (connectionId: string, workspaceId: string, account: FinverseAccount, institutionName?: string, explicitlySelected = false) => {
  ...
  return prisma.$transaction(async tx => {
    ...
    if (!allowance.usedIds.has(account.account_id) && allowance.remaining === 0) throw new PlanQuotaError("Your bank-account allowance is full for this monthly period. You can reconnect an account already used this period.");
    ...
    if (match?.finverseAccountLink && match.finverseAccountLink.externalAccountId !== account.account_id) throw new PlanQuotaError("This Clover account is already linked to a different bank account. Review its link first.");
    ...
    const payload = { connectionId, workspaceId, externalAccountId: account.account_id, accountId, rawPayload: json(account), normalizedPayload: json(normalized), lastSeenAt: new Date(), unlinkedAt: null };
    if (sameIdentity) await tx.finverseAccountLink.update({ where: { id: sameIdentity.id }, data: payload });
    else await tx.finverseAccountLink.create({ data: payload });
    await tx.bankLinkUsage.createMany({ data: [{ userId: workspace.userId, externalAccountId: account.account_id, periodStart: allowance.periodStart, periodEnd: allowance.periodEnd }], skipDuplicates: true });
    return accountId;
  });
};

const importTransaction = async (connectionId: string, workspaceId: string, transaction: FinverseTransaction) => {
  ...
  return prisma.$transaction(async tx => {
    ...
    if (existing) {
      await tx.finverseTransactionRecord.update({ where: { id: existing.id }, data: { rawPayload: json(transaction), normalizedPayload: json(normalizedJson), lastSeenAt: new Date() } });
      return "existing" as const;
    }
    const from = new Date(normalized.date); from.setUTCHours(0, 0, 0, 0);
    const to = new Date(+from + 86400000);
    const candidates = await tx.transaction.findMany({ where: { workspaceId, accountId: link.accountId, currency: normalized.currency, amount: normalized.amount, type: normalized.type as TransactionType, date: { gte: from, lt: to }, finverseTransactionRecord: null }, orderBy: [{ createdAt: "asc" }, { id: "asc" }] });
    ...
    await tx.finverseTransactionRecord.create({ data: { connectionId, externalTransactionId: transaction.transaction_id, externalAccountId: transaction.account_id, transactionId: created.id, rawPayload: json(transaction), normalizedPayload: json(normalizedJson) } });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if ((!selectedIds && newAccounts.length > 0) || chosen.filter(a => !allowance.usedIds.has(a.account_id)).length > allowance.remaining) {
      const cloverAccounts = await prisma.account.findMany({ where: { workspaceId: connection.workspaceId } });
      ...
      return NextResponse.json({ status: "select_accounts", connectionId: connection.id, remaining: allowance.remaining, resetsAt: allowance.periodEnd, accounts: newAccounts.map(a => {
        ...
        return { id: a.account_id, name: `${normalized.name}${normalized.accountNumber ? ` •••• ${normalized.accountNumber.slice(-4)}` : ""}`, reserved: allowance.usedIds.has(a.account_id), existingAccountName: matches.length === 1 ? matches[0].name : null };
      }) });
    }
