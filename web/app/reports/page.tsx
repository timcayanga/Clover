...
export default async function ReportsPage() {
  ...
  const currentWindowTransactions = transactions.filter((transaction) => transaction.date >= thirtyDaysAgo);
  ...
  const currentWindowTransactionsDesc = [...currentWindowTransactions].sort((a, b) => b.date.getTime() - a.date.getTime());
  ...
  transactions
    .filter((transaction) => transaction.date >= sixMonthsAgo)
    .forEach((transaction) => {
  ...
  const importedTransactions = transactions.filter((transaction) => transaction.importFileId);
  const manualTransactions = transactions.filter((transaction) => !transaction.importFileId);
  const uncategorizedTransactions = currentWindowTransactionsDesc.filter(
    (transaction) => !transaction.category?.name || !transaction.merchantClean
  );

  const duplicateGroups = new Map<string, ReportTransaction[]>();
        ...
        <article className="report-card glass">
          ...
          <div className="report-insight-grid">
            <div className="report-insight">
              ...
              <small>{formatCurrency(importedTransactions.reduce((sum, transaction) => sum + Number(transaction.amount), 0))} total</small>
            </div>
            <div className="report-insight">
              <span>Manual transactions</span>
              <strong>{manualTransactions.length}</strong>
              <small>{formatCurrency(manualTransactions.reduce((sum, transaction) => sum + Number(transaction.amount), 0))} total</small>
            </div>
          </div>
