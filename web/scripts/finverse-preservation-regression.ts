...
export const requestBankDisconnect=async()=>{};
export const revokeBankConnection=async()=>{if(!disconnected){await fetch('https://api.prod.finverse.net/login_identity',{method:'DELETE'});links.forEach(l=>l.unlinkedAt=new Date());disconnected=true;}return true;};
...
`;
 ...
 api.reset();await assert.rejects(api.importAccount('c1','w',{...bank,account_number_full:'0099999999999'},'Metrobank',true,'foreign-account'),/Choose an existing account/);
 assert.equal(api.state().creates,0);assert.equal(api.state().links.length,0);
 api.reset();assert.equal(await api.importAccount('c1','w',{...bank,account_number_full:'0099999999999'},'Metrobank',true,'existing'),'existing');assert.equal(api.state().accounts[0].accountNumber,'0012345678901','explicit mapping preserves confirmed number');
