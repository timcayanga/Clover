...
async function main() {
 ...
 const usage:any[]=[];let active=[{externalAccountId:'a'},{externalAccountId:'b'}];
 ...
 assert.equal((await bankLinkAllowance(quotaTx,'u',new Date('2026-09-25'))).remaining,0);active=[];
...
account:{findMany:async()=>accounts,create:async({data})=>{creates++;const a={id:'a'+creates,...data};accounts.push(a);return a;},update:async({where,data})=>Object.assign(accounts.find(a=>a.id===where.id),data)},
...
transaction:{findMany:async({where})=>transactions.filter(t=>t.accountId===where.accountId&&t.currency===where.currency&&t.type===where.type&&Number(t.amount)===where.amount&&t.date>=where.date.gte&&t.date<where.date.lt&&!records.some(r=>r.transactionId===t.id)),create:async({data})=>{const t={id:'new'+transactions.length,createdAt:new Date(),...data};transactions.push(t);return t;}},
...
`;
 ...
 api.reserve();api.disconnect();assert.equal(await api.importAccount('c1','w',bank,'Metrobank'),null);assert.equal(await api.importAccount('c2','w',bank,'Metrobank',true),'existing');await assert.rejects(api.importAccount('c2','w',{...bank,account_id:'bank-c'},'Metrobank',true),/allowance is full/);
