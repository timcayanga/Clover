...
assert.match(reportsSource, /buildReportBalanceSeries/);
assert.match(reportsSource, /reportMoneySeries.map/);
...
assert.match(reportsSource, /<strong className=\{comparisonTone\(incomeDelta\)\}>\{formatCurrency\(currentSummary\.income\)\}<\/strong>/);
assert.match(reportsSource, /<strong className=\{comparisonTone\(spendDelta, true\)\}>\{formatCurrency\(currentSummary\.expense\)\}<\/strong>/);
