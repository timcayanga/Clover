...
async function main() {
  ...
  assert.match(transactionsPage, /label="Dates"[\s\S]*label="Type"[\s\S]*label="Accounts"[\s\S]*label="Categories"[\s\S]*aria-label="Amount Range"[\s\S]*label="Currency"[\s\S]*label="Tags"[\s\S]*label="Source"[\s\S]*label="Extraction confidence"[\s\S]*label="Warnings"/);
