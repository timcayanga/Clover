...
export function TransactionSelectionToolbar({ compact, count, query, onQueryChange, filterOpen, onFilter, onEdit, onTags, onDelete, onClear }: Props) {
  return <div className={`transaction-selection-toolbar${compact ? " transaction-selection-toolbar--compact" : ""}${count ? " transaction-selection-toolbar--selected" : ""}`}>
    {count ? <>
      ...
      <button type="button" className="button button-secondary button-small transactions-action-button transaction-selection-toolbar__filter" onClick={onFilter} aria-label="Filter transactions" aria-expanded={filterOpen}>Filter</button>
    </>}
  </div>;
}
