...
export function TransactionSelectionToolbar({ compact, count, query, onQueryChange, filterOpen, onFilter, onEdit, onTags, onDelete }: Props) {
  ...
  useEffect(() => {
    ...
    const outside = (event: PointerEvent) => {
      if (event.target instanceof Element && !event.target.closest(".transactions-mobile-search, .transaction-selection-toolbar")) setSearchOpen(false);
    };
  ...
  return <div className={`transaction-selection-toolbar${compact ? " transaction-selection-toolbar--compact" : ""}`}>
    {count ? compact ? <TransactionsHeaderOverlay className="transactions-selection-overlay">{actions}</TransactionsHeaderOverlay> : actions : <>
      {compact ? <button ref={trigger} type="button" className="icon-button transaction-selection-toolbar__search" aria-label="Search" aria-expanded={searchOpen} onClick={() => { if (filterOpen) onFilter(); setSearchOpen((value) => !value); }}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><circle cx="10.5" cy="10.5" r="6.5"/><path d="m16 16 5 5"/></svg></button> : search}
