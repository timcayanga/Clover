...
const historyKey = (asset: PortfolioGrowthAsset, range: MarketRange) =>
  `${asset.market}:${asset.symbol}:${range}`;
...
const loadAssetHistory = async (
  ...
  if (!response.ok)
    throw new Error(
      payload.error ?? `Market history is unavailable for ${asset.name}.`,
    );
...
export function InvestmentPortfolioGrowthChart({ assets, currency }: Props) {
  ...
  const hasDatedSelection = selectedAssets.some((asset) =>
    Boolean(asset.startDate),
  );
  ...
  useEffect(() => {
    ...
    const recordedFallbacks = new Map(
      selectedAssets
        ...
        .filter(
          (history): history is PortfolioGrowthHistory => history !== null,
        )
        .map((history) => [history.assetId, history] as const),
    );
    const marketAssets = selectedAssets.filter(
      (asset) => asset.historyMode === "market",
    );
    ...
    void Promise.allSettled(
      marketAssets.map((asset) =>
        loadAssetHistory(asset, range, controller.signal),
      ),
    )
      .then((results) => {
        if (cancelled) return;
        ...
        const marketWindowStart = results
          .flatMap((result) =>
            result.status === "fulfilled" ? result.value.points : [],
          )
        ...
        if (range !== "MAX" && marketWindowStart !== undefined) {
          selectedAssets.forEach((asset) => {
            if (loaded.get(asset.id) !== recordedFallbacks.get(asset.id))
              return;
            const aligned = buildRecordedValueHistory(
              asset,
              range,
              new Date(),
              new Date(marketWindowStart).toISOString(),
            );
        ...
        setError(
          unavailable.length > 0
            ? `No market history yet for ${unavailable.join(", ")}.`
            : "",
        );
      })
      .finally(() => {
  ...
  const sourceCurrencies = useMemo(
    () => histories.map((history) => history.currency),
    [histories],
  );
  ...
  const points = useMemo(
    ...
    [
      assetsWithHistory,
      exchangeRates.rates,
      exchangeRates.unavailable.length,
      histories,
      range,
    ],
  );
        ...
        Array.from({ length: Math.min(points.length, 6) }, (_, index) =>
          Math.round(
            (index * (points.length - 1)) /
              Math.max(Math.min(points.length, 6) - 1, 1),
          ),
        ),
      ),
    ];
  ...
  const handlePointerMove = (clientX: number) => {
    ...
    const chartX = Math.min(
      Math.max(((clientX - rect.left) / rect.width) * chartWidth, 0),
      chartWidth,
    );
  ...
  if (assets.length === 0) {
    return (
      <div className="investments-growth-chart__empty">
        ...
        <span>
          Record a purchase date and current value so Clover knows when to begin
          the chart.
        </span>
      </div>
    );
    ...
    <div className={`portfolio-growth portfolio-growth--${tone}`}>
      <div className="portfolio-growth__controls">
        <DismissibleDetails className="portfolio-growth__asset-picker">
          ...
          <div
            className="portfolio-growth__asset-options"
            role="group"
            aria-label="Investments included in growth chart"
          >
            {assets.map((asset) => (
        ...
        <label className="portfolio-growth__period">
          <span className="sr-only">Investment growth period</span>
          <select
            aria-label="Investment growth period"
            value={range}
            onChange={(event) => {
              setRange(event.target.value as MarketRange);
              setHoverIndex(null);
            }}
          >
      ...
      <div className="portfolio-growth__summary" aria-live="polite">
        <span>
          {hovered
            ? formatChartDate(hovered.date, range)
            : "Selected portfolio"}
        </span>
        <strong>
          {hovered || latestPoint
            ? formatCurrencyAmount(
                (hovered ?? latestPoint)?.value ?? 0,
                currencyCode,
              )
            : "—"}
        </strong>
        {points.length > 1 ? (
          <em className={positive ? "is-good" : "is-danger"}>
            {change >= 0 ? "+" : ""}
            {formatCurrencyAmount(change, currencyCode)}
          </em>
        ) : null}
      </div>
        ...
        <div className="portfolio-growth__plot">
          <div className="portfolio-growth__canvas">
            ...
            >
              ...
              <path
                className="portfolio-growth__line"
                d={chart.linePath}
                fill="none"
              />
              {hovered ? (
                <line
                  className="portfolio-growth__hover-line"
                  x1={hovered.x}
                  x2={hovered.x}
                  y1={chartPadding}
          ...
          <div className="portfolio-growth__axis" aria-hidden="true">
            {tickIndexes.map((index) => (
              <span key={points[index]?.date}>
                {formatChartDate(points[index]?.date ?? "", range)}
              </span>
            ))}
          </div>
        </div>
      ...
      {exchangeRates.unavailable.length > 0 ? (
        <p className="portfolio-growth__notice">
          Currency conversion is unavailable for part of this chart.
        </p>
      ) : null}
      <p className="portfolio-growth__method">
        Market assets use available prices and recorded units. Other investments
        use saved purchase dates, purchase values, and current values.
      </p>
      <p className="portfolio-growth__footnote">
        {selectedIds.length} holdings · {currencyCode} · Includes changes in
