...
export function SplitBillHome({
  ...
  const sharedWith = (bill: SplitBillSerializedBill) => {
    ...
    return <span className="split-bill-shared-with">
      {bill.group ? <SplitBillEntityAvatar name={bill.group.name} avatarUrl={group?.avatarUrl ?? null} /> :
            ...
            {visibleGroups.map((group) => (
              <article className="split-bill-group-card" key={group.id}>
                <div className="split-bill-group-identity">
                  <SplitBillEntityAvatar name={group.name} avatarUrl={group.avatarUrl} sizeClass="split-bill-group-photo" />
