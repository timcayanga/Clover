...
export function BudgetingWorkspace({ initialData }: { initialData: BudgetingData }) {
  ...
  return <CloverShell active="budgeting" title={title}
    ...
    actions={!mobileEditor ? <div className="collection-toolbar-actions"><ContextualAskClover context="budgeting" /><button className="button button-primary button-small accounts-toolbar-add collection-toolbar-action" type="button" aria-label="Create Budget" onClick={() => selectEditor("new")}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><path d="M12 5v14M5 12h14" /></svg><span>Create Budget</span></button></div> : undefined}
  >
