...
const SPECIFIC_MERCHANT_CATEGORY_HINTS: Array<{
  ...
  {
    category: "Shopping",
    lower: /paypal|amazon|alibaba|lazada|shopee|uniqlo|zara|h\s*&\s*m|h&m|watsons|puregold|landers|s\s*&\s*r|snr|duty\s+free|robinsons\s+easymart|uncle\s+john'?s|7-?eleven|robinsons\s+supermarket|the\s+marketplace|sm\s+store|power\s+mac|beyond\s+the\s+box|abenson|ace\s+hardware|true\s+value|bench|penshoppe|miniso|muji|cotton\s+on|savemore|shopwise|waltermart|lawson|sm\s+hypermarket|sm\s+supermarket|rustan'?s|toy\s+kingdom|octagon|mitsukoshi|greenbelt|central\s+embassy|siam\s+paragon|\btops\b|7-?11/,
    compact: /paypal|amazon|alibaba|lazada|shopee|uniqlo|zara|hm|watsons|puregold|landers|snr|dutyfree|robinsonseasymart|unclejohns|7eleven|robinsonssupermarket|themarketplace|smstore|powermac|beyondthebox|abenson|acehardware|truevalue|bench|penshoppe|miniso|muji|cottonon|savemore|shopwise|waltermart|lawson|smhypermarket|smsupermarket|rustans|toykingdom|octagon|mitsukoshi|greenbelt|centralembassy|siamparagon|tops|711/,
  },
];
...
export const getStrongMerchantCategoryHint = (value: string): string | null => {
  ...
  if (matchesCategoryHint(value, { lower: /bruno'?s?\s+barbers?|the\s+spa|wheyl\s+nutrition|mercury\s+drug|drug\s+care|anytime\s+fitness|clinic|hospital|pharmacy|healthy\s+options|southstar|rose\s+pharmacy|generika|the\s+generics|hi-?precision|st\.?\s*luke'?s|medical\s+city|vision\s+express|belo/, compact: /brunosbarbers?|thespa|wheylnutrition|mercurydrug|drugcare|anytimefitness|clinic|hospital|pharmacy|healthyoptions|southstar|rosepharmacy|generika|thegenerics|hiprecision|stlukes|medicalcity|visionexpress|belo/ })) {
  ...
  if (
    matchesCategoryHint(value, {
      lower:
        /bill|utilities|electric|water|internet|phone|subscription|subscriptions|openai|chatgpt|netflix|spotify|linkedin|adobe|canva|icloud|google\s+one|youtube\s+premium|airalo|globe|smart|pldt|meralco|maynilad|kindle\s+(?:unltd|unlimited)|openai\s+api|apple\s+services/,
      compact:
        /bill|utilities|electric|water|internet|phone|subscription|subscriptions|openai|chatgpt|netflix|spotify|linkedin|adobe|canva|icloud|googleone|youtubepremium|airalo|globe|smart|pldt|meralco|maynilad|kindleunltd|kindleunlimited|appleservices/,
    })
  ) {
  ...
  if (
    matchesCategoryHint(value, {
      lower: /relay\b|amazon|alibaba|camera|paypal|dhl[-\s]+duty\s+collection|viator(?:\.com)?|locker\s+hire|emmanuel\s+payments?|shop\b|store\b|mart\b|convenience|provisioning\s+service|visa\s+provisioning\s+service|apple\s+pay|google\s+pay|gift\s+shop|duty\s+free|robinsons\s+easymart|uncle\s+john'?s|7-?eleven|robinsons\s+supermarket|the\s+marketplace|sm\s+store|power\s+mac|beyond\s+the\s+box|abenson|ace\s+hardware|true\s+value|bench|penshoppe|miniso|muji|cotton\s+on|savemore|shopwise|waltermart|lawson|sm\s+hypermarket|sm\s+supermarket|rustan'?s|toy\s+kingdom|octagon/,
      compact: /relay|amazon|alibaba|camera|paypal|dhldutycollection|viator|lockerhire|emmanuelpayments?|shop|store|mart|convenience|provisioningservice|visaprovisioningservice|applepay|googlepay|giftshop|dutyfree|puregold|landers|snr|sandr|uniqlo|zara|h&m|watsons|robinsonseasymart|unclejohns|7eleven|robinsonssupermarket|themarketplace|smstore|powermac|beyondthebox|abenson|acehardware|truevalue|bench|penshoppe|miniso|muji|cottonon|savemore|shopwise|waltermart|lawson|smhypermarket|smsupermarket|rustans|toykingdom|octagon/,
    })
  ) {
    return "Shopping";
  }
